		...
		if event is InputEventMouse:
			...
			if target_object is not NavigableGridMap:
				...
				if level.active_unit and level.active_unit.can_move() and level.active_unit.potential_moves.has(coords):
					if event is InputEventMouseButton and event.is_pressed() and event.button_index == MOUSE_BUTTON_LEFT and level.path_marking_system.path.size() >= 2:
					...
					elif event is InputEventMouseButton and event.is_pressed() and event.button_index == MOUSE_BUTTON_RIGHT and level.path_marking_system.hovered_path.size() >= 2:
