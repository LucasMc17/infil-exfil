# NOTE: This code adapted from antzGames to match my specific needs. Thanks King.
@tool
...
func do_debug_path(start_pos : Vector3i, end_pos : Vector3i):
	...
	DebugDraw2D.set_text.call_deferred("2. Walkable Ids: ", walkable_items, 0, Color(0, 0, 0, 0), INF)
	if path.size() < 1:
	...
	DebugDraw3D.draw_box.call_deferred(temp, Quaternion.IDENTITY, Vector3(path_cell_size, path_cell_size, path_cell_size), Color.GREEN, true, INF)
	temp = map_to_local(end_pos)
	temp.y += path_cell_size
	DebugDraw3D.draw_box.call_deferred(temp, Quaternion.IDENTITY, Vector3(path_cell_size, path_cell_size, path_cell_size), Color.RED, true, INF)
	...
	for next_point: Vector3 in path:
		...
		if i > 0 and i < path.size() - 1: 
			DebugDraw3D.draw_box.call_deferred(points[i], Quaternion.IDENTITY, Vector3(path_cell_size, path_cell_size, path_cell_size), Color.YELLOW, true, INF)
		i += 1
